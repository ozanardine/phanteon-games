...
export async function processPaymentNotification(topic, id) {
  ...
  try {
    ...
    if (paymentInfo.status === 'approved') {
      ...
      // Log da referência externa para diagnóstico
      ...
      if (externalReference.includes('|')) {
        ...
        console.log(`[MercadoPago] Referência alternativa - userId: ${userId}, planId: ${planId}`);
      }
      
      if (!userId || !planId) {
        console.error('[MercadoPago] Falha ao extrair userId/planId:', { externalReference });
        // Registra mais detalhes do paymentInfo para diagnóstico
        console.error('[MercadoPago] Detalhes do pagamento para diagnóstico:', {
    ...
    console.error('[MercadoPago] Detalhes do erro:', {
      message: error.message,
      stack: error.stack,
      topic,
      id,
      // Se o erro tiver informações adicionais da API
      cause: error.cause,
      status: error.status
    });
