...
/**
 * Cria um usuário no sistema de autenticação do Supabase
 * @param {Object} userData - Dados do usuário
 * @returns {Promise<Object>} - Dados do usuário criado
 */
export async function createSupabaseUser(email, password = null) {
  if (!supabaseAdmin) {
    console.error('Cliente Supabase Admin não inicializado');
    return null;
  }
  ...
  try {
    ...
    const randomPassword = password || Math.random().toString(36).slice(-10) + 
