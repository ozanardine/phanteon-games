...
export async function getUserSubscription(userId) {
  ...
  try {
    ...
    // Buscar qualquer assinatura com status 'active', independente da data de expiração
    // Isso garante que uma assinatura recém-aprovada seja detectada imediatamente
    const { data, error } = await supabaseAdmin
      .from('subscriptions')
      .select('*, plans:plan_id(name)')  // Incluir informações do plano quando possível
      .eq('user_id', userId)
      .eq('status', 'active')
    ...
    console.log(`[Supabase] Assinatura ativa encontrada: ${data.id}, status: ${data.status}, plano: ${data.plans?.name || data.plan_id}`);
    return data;
  } catch (error) {
